...
\begin{enumerate}
    \item \textbf{Storage:} Persistence files (\textasciitilde{}2\,MB each) stored in the bot's PostgreSQL database, eliminating the need for AWS S3, credentials, or a standalone sync app.
    \item \textbf{Pre-seed on connect:} A Lua script in the pilot's \texttt{Scripts/Hooks/} folder (one-time install, like SRS) downloads \emph{all} squadron cache files when the pilot connects to the server. Files are in place before anyone slots in.
    \item \textbf{Upload on deslot:} When the pilot unslots or disconnects, the hook uploads \emph{only} the flown aircraft's updated cache file back to the bot API.
    \item \textbf{Invisible to pilots:} No app to open, no buttons to click. Persistence sync is fully automatic.
\end{enumerate}
...
\begin{description}[style=nextline,leftmargin=0.5em]
    \item[Beta] Client hook for automated persistence sync. PostgreSQL binary storage with version history. Role-based permissions (pilot vs.\ engineering vs.\ admin). Flight hour thresholds for scheduled maintenance triggers.
