...
The staging server tracks upstream development with MCP tooling cherry-picked on top:

```bash
# Update staging to latest upstream + cherry-pick pending PRs
ssh hoverstop "cd C:\Users\Administrator\github\DCSServerBot && git fetch upstream development && git reset --hard upstream/development"

# Cherry-pick pending PRs (get commit hashes from fork branches)
ssh hoverstop "cd C:\Users\Administrator\github\DCSServerBot && git fetch https://github.com/engines-wafu/DCSServerBot.git <branch> && git cherry-pick FETCH_HEAD"

# Cherry-pick MCP API plugin (staging only, not for upstream)
ssh hoverstop "cd C:\Users\Administrator\github\DCSServerBot && git cherry-pick <mcpapi-commit>"

# Clear pycache and restart
ssh hoverstop 'powershell "Remove-Item -Recurse -Force C:/Users/Administrator/github/DCSServerBot/plugins/*/__pycache__ 2>$null; New-Item -Path C:/Users/Administrator/github/DCSServerBot/.restart_requested -ItemType File -Force"'

# Verify health
sleep 15 && curl -s http://192.154.225.181:9876/mcp/bot/health
```

**Note:** The MCP API plugin is staging-only (not submitted upstream). It enables:
- Execute slash commands via REST API for testing
- Restart the bot remotely without RDP
- Check bot health and logs programmatically
...
### Plugin Status (Merged to Upstream)

All three plugins are now merged into upstream development branch:

- **FlightPlan Plugin** - IFR-style flight planning with F10 map visualization, waypoint autocomplete, OpenAIP integration, auto-lifecycle (activate on takeoff, complete on landing)
...
python plugins/logbook/db/migrate_legacy.py \
  ...
  --slmod-stats /path/to/combinedStats.json \
  --dry-run
```

See migration plan: `~/.claude/plans/jsw-data-migration-plan.md`
...
1. **Migrations don't auto-run if version matches** - If the `plugins` table shows a version >= migration file version, the migration won't run. Manual ALTER TABLE may be needed:
   ```sql
   -- Check current plugin version
   SELECT * FROM plugins WHERE plugin = 'logistics';

   -- Manual fix example (created_by_ucid nullable)
   ALTER TABLE logistics_tasks ALTER COLUMN created_by_ucid DROP NOT NULL;
   ```

2. **Schema validation warnings** - Add new config keys to `schemas/<plugin>_schema.yaml` to avoid startup warnings.

### F10 Menu Notes

- Multiple plugins can create F10 menus (after PR #129 merge)
- Each plugin should use a unique root menu name (e.g., "Flight Plan", "Logistics")
- Lua changes require DCS mission restart (not just bot restart)

### Discord Slash Command Quirks

**Auto-hidden autocomplete parameters:** When a slash command parameter is required, uses autocomplete, and has only ONE valid choice, Discord will hide the parameter from the UI and auto-fill it behind the scenes. This is Discord's intended UX behavior, not a bug.

Impact on testing:
- **Single-server setups:** The `server` parameter disappears from commands like `/logistics list` or `/flightplan list` because there's only one valid choice
- **Multi-server setups:** The parameter appears normally with all choices visible

This can cause confusion during development on single-server staging environments. The parameter is still being passed correctly — it's just not visible in the Discord UI.

### Cherry-Pick Workflow for Urgent Fixes

When PRs are pending but fixes are needed immediately:
```bash
# On production server, add fork remote
git remote add fork https://github.com/engines-wafu/DCSServerBot.git

# Fetch and cherry-pick
git fetch fork
git cherry-pick <commit-hash>

# Clear pycache for affected plugins
Remove-Item -Recurse -Force plugins/<plugin>/__pycache__
...
**Important:** Always read recent messages from the channel before posting updates or summaries. Use `mcp__discord__read-messages` to check for user feedback, questions, or confirmations that may need to be addressed or acknowledged in your message.
...
```
/flightplan fix sync theater:Afghanistan
/flightplan fix seed theater:Afghanistan
```
Both commands are required - `sync` fetches from OpenAIP API, `seed` loads bundled DCS-specific fixes.
